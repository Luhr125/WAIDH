...
/*
  NO SAFE STEP – SPIELLOGIK

  JavaScript übernimmt hier vier Hauptaufgaben:
  1. Es speichert den aktuellen Spielzustand.
  2. Es bewegt Spieler und Gegner.
  3. Es prüft Kollisionen, Münzen, Checkpoints und Raumwechsel.
  4. Es zeichnet das komplette Spiel für jeden neuen Bildschirm-Frame.

  Hilfreiche JavaScript-Zeichen:
  const  = Variable, die nicht neu zugewiesen werden soll
  let    = Variable, deren Wert sich verändern darf
  ...
  ||     = oder
  !      = nicht
  =>     = kurze Schreibweise für eine Funktion
*/
...
/*
  DRAW – ZEICHNET DEN AKTUELLEN SPIELZUSTAND

  Das Canvas speichert keine einzelnen Spielobjekte wie HTML-Elemente. Deshalb wird
  in jedem Frame das gesamte Bild neu aufgebaut: Hintergrund, Raster, Wände, Münzen,
  Gegner, Projektile, Checkpoint, Spieler und zuletzt die Textanzeige.
*/
function draw(){
  ...
  // Nur noch nicht eingesammelte Münzen werden gezeichnet.
  ...
  // Jeder Gegnertyp erhält eine eigene Form: Kreis, Turret, Mine oder Drehbalken.
  ...
  // Grüner Checkpointbereich für die beiden Spieler.
  ...
  // P1 wird gelb, P2 blau gezeichnet; anschließend kommt die Spielernummer darauf.
  ...
  // Raumname sowie aktueller und gespeicherter Münzstand.
...
/*
  Hauptschleife des Spiels. requestAnimationFrame ruft loop ungefähr passend zur
  Bildrate des Monitors erneut auf. dt wird auf 0,035 Sekunden begrenzt, damit nach
  einem kurzen Hänger keine Figur plötzlich eine riesige Strecke überspringt.
*/
function loop(t){const dt=Math.min((t-last)/1000,.035);last=t;update(dt);updateBarrierStatus();draw();requestAnimationFrame(loop);}

// Tastendrücke im Set speichern; Standardaktionen der Pfeiltasten werden verhindert.
// Beim Loslassen wird die Taste entfernt, und bei Fokusverlust werden alle gelöscht.
...
// Startzustand: keine Münzen/Tode, Raum 1 von links betreten und Spielschleife starten.
